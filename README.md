<div align="center">
  ...
  <img alt="版本" src="https://img.shields.io/badge/version-1.2.0-536af5">
...
- **视频先字幕、后深读**：首次触发不打开浮窗，优先在原播放器开启页面 CC；没有可用字幕时可转写音轨。谷歌临时译文从当前播放位置持续生成，AI 译文完成一条就无感接管一条。只有随后点击“解析视频内容”，才会提取关键帧并生成整体解读。
...
- 分层展示原文、中文翻译和含义；按内容语义用 Markdown 重建标题、段落、列表、引用、表格与代码层级，再转换为安全、简约的 HTML 阅读版式。
...
- 谷歌临时译文从当前播放位置开始持续生成，AI 流式译文完成一条就覆盖一条；AI 完成后停止临时翻译，最终历史只保存 AI 译文。
